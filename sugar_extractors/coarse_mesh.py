...
def extract_mesh_from_coarse_sugar(args):
    ...
    # Pruning low opacity gaussians
   # === [STEP 1] 终极过滤与对齐点云导出 ===
    # === [STEP 1] 核心过滤：删掉白云种子，保留天线种子 ===
    # === [STEP 1] 终极优化：参数过滤 + 离群点剔除 ===
    with torch.no_grad():
        CONSOLE.print("\n[FINAL OPTIMIZATION] Purifying Antenna Structure...")
        
        low_opacity_threshold = 0.39   # 提升：确保材质更坚实
        max_scale_threshold = 0.08   # 压低：彻底消除大块浮窗
        center_bound = 2.5 
        
        scales = torch.exp(sugar._scales)
        s_mask = (sugar.strengths.squeeze() > low_opacity_threshold).cpu()
        sc_mask = (scales.max(dim=-1).values.squeeze() < max_scale_threshold).cpu()
        spatial_mask = (sugar._points.abs() < center_bound).all(dim=-1).cpu()
        
        combined_mask = s_mask & sc_mask & spatial_mask
        combined_mask = combined_mask.to(sugar._points.device)
        
        # --- 秘密武器：离群点过滤 (SOR) ---
        import open3d as o3d
        # 先进行初步过滤提取
        tmp_points = sugar._points[combined_mask].detach().cpu().numpy()
        if len(tmp_points) > 0:
            pcd_tmp = o3d.geometry.PointCloud()
            pcd_tmp.points = o3d.utility.Vector3dVector(tmp_points)
            
            # nb_neighbors=30: 检查每个点周围的 30 个邻居
            # std_ratio=1.0: 越小越严格。如果一个点离群众太远，直接开除
            cl, ind = pcd_tmp.remove_statistical_outlier(nb_neighbors=30, std_ratio=1.0)
            
            # 这里的 ind 是 combined_mask 内部的索引，我们需要重新映射回原点集
            raw_indices = torch.where(combined_mask)[0]
            final_keep_indices = raw_indices[ind]
        ...
        # 彻底更新参数
        sugar._points = torch.nn.Parameter(sugar._points[final_mask])
        sugar._scales = torch.nn.Parameter(sugar._scales[final_mask])
        sugar._quaternions = torch.nn.Parameter(sugar._quaternions[final_mask])
        sugar.all_densities = torch.nn.Parameter(sugar.all_densities[final_mask])
        if hasattr(sugar, '_sh_coordinates_dc'):
            sugar._sh_coordinates_dc = torch.nn.Parameter(sugar._sh_coordinates_dc[final_mask])
        if hasattr(sugar, '_sh_coordinates_rest'):
            sugar._sh_coordinates_rest = torch.nn.Parameter(sugar._sh_coordinates_rest[final_mask])
            
        CONSOLE.print(f"Number of gaussians after Statistical Cleaning: {len(sugar._points)}")# --- 4. 关键：修正颜色提取逻辑并导出 PLY ---
        import open3d as o3d
        import numpy as np
        final_pcd = o3d.geometry.PointCloud()
        
        # 提取点坐标
        final_pcd.points = o3d.utility.Vector3dVector(sugar._points.detach().cpu().numpy())
        
        # --- 重点修改：手动将 SH 系数转为 RGB 颜色 ---
        # 提取 DC 分量 (Shape: [N, 1, 3] 或 [N, 3])
        sh_dc = sugar._sh_coordinates_dc.detach()
        if sh_dc.dim() == 3:
            sh_dc = sh_dc.squeeze(1)
            
        # 使用 3DGS 的球谐函数线性变换公式 (SH2RGB)
        # 0.28209 是 SH 的基础系数
        C0 = 0.28209479177387814
        final_colors_raw = (sh_dc * C0 + 0.5).clamp(0, 1).cpu().numpy()
        final_pcd.colors = o3d.utility.Vector3dVector(final_colors_raw)
        
        # 确保输出目录存在并保存
        os.makedirs(args.mesh_output_dir, exist_ok=True)
        pcd_path = os.path.join(args.mesh_output_dir, "clean_aligned_antenna_skeleton.ply")
        o3d.io.write_point_cloud(pcd_path, final_pcd)
        CONSOLE.print(f"SUCCESS: Clean aligned point cloud saved at: {pcd_path}")
        with torch.no_grad():
            # 获取所有高斯点的 XYZ 坐标
            points = sugar._points
            
            # 计算掩码：保留 XYZ 坐标绝对值都在范围内的点
            # points.abs() < center_bound 会得到一个 [N, 3] 的布尔矩阵
            # .all(dim=-1) 要求 X、Y、Z 三个方向都必须在范围内
            keep_mask = (points.abs() < center_bound).all(dim=-1)
            
            CONSOLE.print(f"Number of gaussians after spatial filtering: {keep_mask.sum().item()}")
              
            # 依照掩码更新 sugar 模型的内部所有参数
            sugar._points = torch.nn.Parameter(sugar._points[keep_mask])  
            sugar._scales = torch.nn.Parameter(sugar._scales[keep_mask])  
            sugar._quaternions = torch.nn.Parameter(sugar._quaternions[keep_mask])  
            sugar.all_densities = torch.nn.Parameter(sugar.all_densities[keep_mask])  
              
            # 同步更新颜色参数（SH）  
            if hasattr(sugar, '_sh_coordinates_dc'):  
                sugar._sh_coordinates_dc = torch.nn.Parameter(sugar._sh_coordinates_dc[keep_mask])  
            if hasattr(sugar, '_sh_coordinates_rest'):  
                sugar._sh_coordinates_rest = torch.nn.Parameter(sugar._sh_coordinates_rest[keep_mask])
            
        
        CONSOLE.print(f"Number of gaussians after scale filtering: {sugar.n_points}")
        # === 物理特性过滤结束 ===
    ...
    if not use_marching_cubes:
        if not use_centers_to_extract_mesh:
            ...
            for surface_level in surface_levels:
                ...
                # === 准话修改二：导出高质量皮肤点云 ===
                import open3d as o3d
                # 直接从当前 surface_level 的输出字典中获取点和颜色
                save_pcd_points = surface_levels_outputs[surface_level]['points']
                save_pcd_colors = surface_levels_outputs[surface_level]['colors']
                
                # 为了防止点云太大导致查看器卡顿，保留 200 万个点就足够厚实了
                if len(save_pcd_points) > 2000000:
                   
                    indices = torch.randperm(len(save_pcd_points))[:2000000]
                    save_pcd_points = save_pcd_points[indices]
                    save_pcd_colors = save_pcd_colors[indices]

                # 构建 Open3D 点云对象
                skin_pcd = o3d.geometry.PointCloud()
                skin_pcd.points = o3d.utility.Vector3dVector(save_pcd_points.detach().cpu().numpy())
                skin_pcd.colors = o3d.utility.Vector3dVector(save_pcd_colors.detach().cpu().numpy())
                
                # 保存皮肤点云到输出目录
                pcd_skin_path = os.path.join(args.mesh_output_dir, f"high_quality_antenna_skin_level_{surface_level}.ply")
                os.makedirs(args.mesh_output_dir, exist_ok=True)
                o3d.io.write_point_cloud(pcd_skin_path, skin_pcd)
                CONSOLE.print(f"!!! SUCCESS: Dense skin point cloud saved at: {pcd_skin_path}")

                # 原有逻辑：继续为后续的 Mesh 重建准备数据
                ...
                if fg_points.shape[0] > 0:
                    ...
                    fg_pcd = fg_pcd.select_by_index(ind)
                      ...
                      # === 新增：下采样逻辑，这一行最关键 ===
                    CONSOLE.print("Voxel downsampling to prevent lag...")
                    fg_pcd = fg_pcd.voxel_down_sample(voxel_size=0.01) 
                    # ===================================
                    ...
                    o3d_fg_mesh, o3d_fg_densities = o3d.geometry.TriangleMesh.create_from_point_cloud_poisson(
                        fg_pcd, depth=10) #, width=0, scale=1.1, linear_fit=False)  # depth=10 should be the default value? 11 is good to (but it starts to make a big number of triangles)
                ...
                if bg_points.shape[0] > 0:
                    ...
                    o3d_bg_mesh, o3d_bg_densities = o3d.geometry.TriangleMesh.create_from_point_cloud_poisson(
                        bg_pcd, depth=10) #, width=0, scale=1.1, linear_fit=False)  # depth=10 should be the default value? 11 is good to (but it starts to make a big number of triangles)
